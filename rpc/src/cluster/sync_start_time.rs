...
impl<C: Config> RpcParameter<C> for SyncStartTime<C::Signature, C::Address> {
    ...
    async fn handler(self, ctx: C) -> RpcResult<Self::Response> {
        ...
        if let Some(sender) = self.0.sender() {
            ...
            let maybe_leader =
                ctx.verify_signature(&self.0.signature, &self.0.commitment, Some(sender))?;
            let (leader_address, _) = ctx
            ...
            ctx.async_task()
                .emit_event(SessionEvent::GenesisSession(start_time).into())
                .await?;

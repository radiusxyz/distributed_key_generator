...
use dkg_primitives::{AppState, SessionId, SyncDecryptionKeyPayload, KeyGeneratorList, AggregatedKey, DecryptionKey, AsyncTask};
...
pub fn broadcast_decryption_key_ack<C: AppState>(
    ...
    ctx.async_task().multicast(committee_urls, <SyncDecryptionKey::<C::Signature, C::Address> as RpcParameter<C>>::method().into(), SyncDecryptionKey { signature, payload });

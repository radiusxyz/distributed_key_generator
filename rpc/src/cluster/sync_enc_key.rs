...
impl<C: Config> RpcParameter<C> for SyncEncKey<C::Signature, C::Address> {
    ...
    async fn handler(self, ctx: C) -> RpcResult<Self::Response> {
        if let Some(sender) = self.sender() {
            ...
            if sender == ctx.address() {
                return Ok(());
            }
            let _ =
                ctx.verify_signature(&self.0.signature, &self.0.commitment, Some(sender.clone()))?;
            let enc_key_commitment = self
                .0
                .commitment
                .payload
                .decode::<EncKeyCommitment<C::Signature, C::Address>>()?;

...
use crate::{rpc::{default_external_rpc_server, default_cluster_rpc_server}, DkgWorker, run_dkg_worker};
...
pub async fn run_node<C: AppState>(ctx: &mut C, config: Config, rx: Receiver<Event<C::Signature, C::Address>>) -> Result<Vec<JoinHandle<()>>, C::Error> {
    ...
    let key_generator_worker = DkgWorker::<C>::new(solver_cluster_rpc_url, config.session_cycle, rx);
    let cloned_ctx = ctx.clone();
    let worker_handle = ctx.async_task().spawn_task(async move {
        if let Err(e) = run_dkg_worker(&cloned_ctx, Arc::new(key_generator_worker)).await {

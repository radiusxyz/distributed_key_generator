...
pub fn init_genesis_session<C: Config>(
    ...
    let urls = operators.iter().filter(|o| {
        if !should_force_generating {
            o.address() != ctx.address()
        } else {
            true
        }
    }).map(|o| o.external_rpc_url().to_string()).collect::<Vec<_>>();
        ...
        .await {
            ...
            if let Some(sender) = commitment.commitment.sender.clone() {    
                ...
                if maybe_committee != registered_address {
                    return Err(OperatorServiceError::AnyError("Sender is not in the committee".to_string()).into());
                }
            } else {
...
pub fn sync_start_time<C: Config>(ctx: &C, start_time: u128, operators: &Vec<Operator<C::Address>>) -> Result<(), C::Error> {
    ...
    let urls = operators.iter().filter(|o| o.address() != ctx.address()).map(|o| o.cluster_rpc_url().to_string()).collect::<Vec<_>>();
...
pub async fn sync_finalized_enc_keys<C: Config>(
    ...
    let mut urls = operators.iter().map(|o| o.cluster_rpc_url().to_string()).collect::<Vec<_>>();

...
pub async fn run_session_worker<C, SW>(
    ctx: C,
    worker: &mut SW,
    ...
    SW: SessionWorker<C>,
{
    ...
    worker.on_genesis_session(ctx.clone()).await?;
    loop {
        let session_info = sessions.next_session().await;
        tracing::info!("Session info: {:?}", session_info.session_id);
        let _ = worker.on_session(ctx.clone(), session_info).await;
    }
}
...
impl SessionInfo {
    pub fn new(session_id: SessionId, duration: Duration) -> Self {
        Self {
            session_id,
            duration,
            ends_at: Instant::now() + time_until_next_session(duration),
        }
    }
}
    ...
    pub async fn next_session(&mut self) -> SessionInfo {
        loop {
            ...
            let current_session = match SessionId::get() {
                ...
                Err(_) => {
                    tracing::error!("Error getting session id");
                    continue;
                }
            };

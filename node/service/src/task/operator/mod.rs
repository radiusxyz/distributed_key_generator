...
use dkg_node_operator::BAppEvent;
use dkg_rpc::DbManager;
use tokio::sync::mpsc;

pub async fn start_blockchain_operator_worker<C: Config>(config: &C, blockchain_event_rx: mpsc::Receiver<Result<BAppEvent, OperatorServiceError>>) -> Result<tokio::task::JoinHandle<()>, C::Error>
where
...
pub struct BlockchainOperatorWorker<C: Config> {
    pub config: C,
    pub blockchain_event_rx: mpsc::Receiver<Result<BAppEvent, OperatorServiceError>>,
}
...
where
    TrustedSetupFor<C>: From<OperatorTrustedSetupFor<C>>,
{
    pub fn new(config: C, blockchain_event_rx: mpsc::Receiver<Result<BAppEvent, OperatorServiceError>>) -> Self {
    ...
    pub async fn handle_task_completed(&mut self) {

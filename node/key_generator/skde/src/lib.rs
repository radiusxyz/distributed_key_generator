    ...
    H::Output: AsRef<[u8]>,
{
    ...
    ) -> KeyServiceResult<Vec<u8>> {
        // If enc_keys are provided, aggregate them
        let params = self.get_params()?;
        let enc_key = if let Some(enc_keys) = maybe_enc_keys {
            let partial_keys = enc_keys
                .iter()
                .map(|key| {
                    serde_json::from_slice::<PartialKey>(key).map_err(KeyGeneratorError::from)
                })
                .collect::<Result<Vec<PartialKey>, KeyGeneratorError>>()?;
            ...
            let enc_key = aggregate_key(params, &selected_keys);
            serde_json::to_vec(&enc_key)?
        } else {
            let (_, partial_key) = generate_partial_key(params)
    ...
    pub fn verify_dec_key(&self, enc_key: &Vec<u8>, dec_key: &Vec<u8>) -> KeyServiceResult<()> {
        ...
        if decrypted_message.as_str() != sample_message {
            return Err(KeyGeneratorError::MessageMismatch);
        }
        tracing::info!("Decryption key verified");
        Ok(())
    }

    fn select_ordered_indices(
        &self,
        n: usize,
        randomness: &Vec<u8>,
    ) -> KeyServiceResult<Vec<usize>> {
        ...
        for i in (1..n).rev() {
            ...
            let output: [u8; 32] = hash.as_ref().try_into().map_err(|_| {
                KeyGeneratorError::InternalError("Failed to convert hash to 32 bytes".into())
            })?;
    ...
    fn select_random_partial_keys(
        &self,
        partial_keys: &Vec<PartialKey>,
        randomness: &Vec<u8>,
    ) -> KeyServiceResult<Vec<PartialKey>> {
    ...
    fn derive_partial_key(&self, selected_keys: &Vec<PartialKey>) -> KeyServiceResult<PartialKey> {
        ...
        let gen = |label: &[u8]| -> KeyServiceResult<BigUint> {
            ...
            let hash: [u8; 32] = hash.as_ref().try_into().map_err(|_| {
                KeyGeneratorError::InternalError("Failed to convert hash to 32 bytes".into())
            })?;
        ...
        // y, w = g^k, g^{k * n} * (1 + n)^r
        let yw_pair = generate_uv_pair(params, &k_h, &r_h)
            .expect("Failed to generate YW pair for partial key");

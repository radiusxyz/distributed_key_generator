...
use dkg_primitives::{Hasher, KeyGenerator, KeyGeneratorError};
pub use skde::{
...
pub type KeyServiceResult<T> = Result<T, KeyGeneratorError>;
    ...
    H::Output: AsRef<[u8]>,
{
    ...
    pub fn gen_enc_key(&self, randomness: Vec<u8>, maybe_enc_keys: Option<Vec<Vec<u8>>>) -> KeyServiceResult<Vec<u8>> {
        ...
        let enc_key = if let Some(enc_keys) = maybe_enc_keys {
            ...
            let partial_keys = enc_keys.iter().map(|key| serde_json::from_slice::<PartialKey>(key).map_err(KeyGeneratorError::from)).collect::<Result<Vec<PartialKey>, KeyGeneratorError>>()?;
            ...
            let (_, partial_key) = generate_partial_key(&self.params).map_err(|e| KeyGeneratorError::InternalError(e.to_string()))?;
    ...
    pub fn gen_dec_key(&self, enc_key: &Vec<u8>) -> KeyServiceResult<(Vec<u8>, u128)> {
        ...
        let secure_key = solve_time_lock_puzzle(&self.params, &enc_key).map_err(|e| KeyGeneratorError::InternalError(e.to_string()))?;
    ...
    pub fn verify_dec_key(&self, enc_key: &Vec<u8>, dec_key: &Vec<u8>) -> KeyServiceResult<()> {
        ...
        if decrypted_message.as_str() != sample_message { return Err(KeyGeneratorError::MessageMismatch); }
    ...
    fn select_ordered_indices(&self, n: usize, randomness: Vec<u8>) -> KeyServiceResult<Vec<usize>> {
        ...
        for i in (1..n).rev() {
            ...
            let output: [u8; 32] = hash.as_ref().try_into().map_err(|_| KeyGeneratorError::InternalError("Failed to convert hash to 32 bytes".into()))?;
    ...
    fn derive_partial_key(&self, selected_keys: &Vec<PartialKey>) -> KeyServiceResult<PartialKey> {
        ...
        let gen = |label: &[u8]| -> KeyServiceResult<BigUint> {
            ...
            let hash: [u8; 32] = hash.as_ref().try_into().map_err(|_| KeyGeneratorError::InternalError("Failed to convert hash to 32 bytes".into()))?;
...
impl<H> KeyGenerator for Skde<H> 
where 
    H: Hasher,
    H::Output: AsRef<[u8]>,
{
    ...
    type Error = KeyGeneratorError; 

    fn setup(param: SkdeParams) -> Self {
        Skde::<H>::new(param)
    }

    fn update_trusted_setup(&mut self, trusted_setup: SkdeParams) -> KeyServiceResult<()> {
        self.params = trusted_setup;
        Ok(())
    }

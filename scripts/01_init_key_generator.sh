...
for (( node_index=0; node_index<NODE_COUNT; node_index++ )) do
    ...
    sed -i.temp "s/external_rpc_url = \"http:\/\/127.0.0.1:3000\"/external_rpc_url = \"http:\/\/$HOST:710$node_index\"/g" $config_file_path
    sed -i.temp "s/internal_rpc_url = \"http:\/\/127.0.0.1:4000\"/internal_rpc_url = \"http:\/\/$HOST:720$node_index\"/g" $config_file_path
    sed -i.temp "s/cluster_rpc_url = \"http:\/\/127.0.0.1:5000\"/cluster_rpc_url = \"http:\/\/$HOST:730$node_index\"/g" $config_file_path

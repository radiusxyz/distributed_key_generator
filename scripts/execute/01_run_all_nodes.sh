...
"$PROJECT_ROOT_PATH/scripts/execute/cleanup_nodes.sh"

# 2. Start authority
echo "[2/4] Starting authority node..."
"$PROJECT_ROOT_PATH/scripts/execute/00_run_authority.sh" &
sleep 1.5  # Give it time to bind the port

# 3. Start leader
echo "[3/4] Starting leader node..."
"$PROJECT_ROOT_PATH/scripts/execute/01_run_leader.sh" &
sleep 1.5

# 4. Start committee (node2)
echo "[4/4] Starting committee node..."
"$PROJECT_ROOT_PATH/scripts/execute/02_run_committee.sh" &
sleep 1.5
